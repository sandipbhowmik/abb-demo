...
> **Scope:** This document describes the target architecture that is provisioned by the Terraform codebase (`tf-az-infraprovison`) to build the required infrastructure for the deployment of Spring PetClinic application microservices on Microsoft Azure. The reference taken from https://github.com/Azure-Samples/java-on-aca where it used Azure Container Apps. This project covers Azure resources, networking, identity & RBAC, security, scalability etc. All names are parameterized by `var.name_prefix` unless otherwise stated.
